import { BsCodeSquare, BsFillGearFill, BsGear, BsInfoLg } from "react-icons/bs";
...
export default function DataInfos({
  id,
  type,
  data,
  isConnectable,
  ...
  const [name, setName] = useState(
    data?.config?.name ?? data.name ?? "Requête " + id
  );
    ...
    <div className="flex  flex-col justify-center items-center space-y-2">
      <div className="text-[8px] text-center font-[PoppinsBold]">
        <div className="mb-[-6px]">
          <span className="text-[6px]  font-[PoppinsBold]">Stat descriptive</span>
        </div>
      </div>
      <div
        className={` ${isAnimate ? "animatecss-headShake" : ""
          } relative text-xs p-[5px] animatecss bg-zinc-800 w-9 h-9 items-center flex justify-center rounded-full text-white`}
      >
        <BsInfoLg size={16} />
        ...
        <div
          ...
          className={`absolute border border-slate-white ${isValidate ? "bg-[#fff] hover:bg-[#eee]" : "bg-[#ccc]"
