...
export default function ArpuNode({ id, type, data, isConnectable }: any) {
    ...
    try {
      if (config != null) {
        ...
        response = await Runner.get(
          `/arpu-${data.config.arpuType}${data.config.operateur == null
            ? ""
            : "?operateurTag=" + config?.operateur?.tag}`
        );
  ...
  return (
    <div className="flex flex-col justify-center items-center">
      <div className="text-[8px] text-center font-[PoppinsBold]">
        {config?.tableSelected?.name || "A.R.P.U"}
        <div className="mt-[-3px]">
          <span className="text-[6px] font-[Poppins]">
            {" "}
            {config?.arpuType + " - " || ""} {config?.operateur?.tag || ""}{" "}
          </span>
        </div>
      </div>
      <div
      ...
      >
        ...
        <div
          ...
          className="absolute rounded no-drag cursor-pointer hover:bg-yellow-600 w-6 h-4 items-center justify-center flex bottom-[-11px] right-[18px] z-1 bg-yellow-500 text-slate-800  text-xs"
        >
