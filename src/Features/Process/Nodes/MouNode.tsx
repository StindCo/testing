...
export default function MouNode({ id, type, data, isConnectable }: any) {
    ...
    try {
      if (config != null) {
        ...
        response = await Runner.get(
          `/mou-${data.config?.mouType}${
            data.config?.operateur == null
              ? ""
  ...
  return (
    <div className="flex flex-col justify-center items-center">
      <div className="text-[8px] text-center font-[PoppinsBold]">
        {config?.tableSelected?.name || "M.O.U"}
        <div className="mt-[-3px]">
          <span className="text-[6px] font-[Poppins]">
            {" "}
            {config?.mouType + " - " || ""} {config?.operateur?.tag || ""}{" "}
          </span>
        </div>
      </div>
      <div className={` ${
        ...
        <div
          ...
          className="absolute rounded no-drag cursor-pointer hover:bg-violet-700 w-6 h-4 items-center justify-center flex bottom-[-11px] right-[18px] z-1 bg-violet-600 text-slate-800  text-xs"
        >
