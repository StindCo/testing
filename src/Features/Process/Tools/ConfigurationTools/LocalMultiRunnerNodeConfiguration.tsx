...
export default function LocalMultiRunnerNodeConfiguration({ details }: any) {
  const params = useContext(ParamsContext);
  const [tableSelected, setTableSelected] = useState<any>();
  const [canSave, setCanSave] = useState(false);
  const [isNameModalShow, setIsNameModalShow] = useState(false);
  const [data_a, setData_a] = useState<any>(details.data?.input_a);
  const [data_b, setData_b] = useState<any>(details.data?.input_b);
  const [dataLoaded, setDataLoaded] = useState<any[]>(data_a);
  const [isAnimate, setIsAnimate] = useState(false);
  const [name, setName] = useState(
    details?.data?.name ?? "Requête " + details.id
  );

  const [code, setCode] = useState(details?.data?.code ?? "");
  ...
  const execute = () => {
    try {
      let result: any = alasql(`${filterCode(code, params)}`, [data_a, data_b]);
